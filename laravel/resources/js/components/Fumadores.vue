<template>
  <div v-loading="loading">
    ...
    <el-row style="font-weight: 600; text-align: center;">
      ...
      <el-col :xs="3" :sm="3" :md="3" :lg="3" :xl="3">
        Fumador
        <i class="el-icon-top" @click="sortList('fumador', 'up')" />
        <i class="el-icon-bottom" @click="sortList('fumador', 'down')" />
      </el-col>
      <el-col :xs="3" :sm="3" :md="3" :lg="3" :xl="3">
        Prior.
...
<script>
  export default {
    ...
    computed: {
      pacientesFiltered() {
        ...
        if (this.filters.edad) {
          list = list.filter((c) => {
            return c['edad'].toString().toString().match(this.filters.edad)
          })
        }
        if (this.filters.fumador) {
          list = list.filter((c) => {
            return c['fumador'] > 0
          })
        }
    ...
    methods: {
      /* getDataInicio() {
        this.axios
          .get(`http://localhost:30019/api/pacientes`, { tabs: 'fumadores'} )
            .then((response) => {
