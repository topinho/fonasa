<template>
  <div v-loading="loading">
    <el-divider>
      Pacientes ordenados por Urgencia (Riesgo)
    </el-divider>
    ...
    computed: {
      pacientesFiltered() {
        ...
        if (this.filters.numero_historia_clinica) {
          list = list.filter((c) => {
            return c['numero_historia_clinica'].match(this.filters.numero_historia_clinica)
          })
        }
        ...
        if (this.filters.item) {
          list = list.filter((c) => {
            return this.filters.item === c['tipo_item_id']
          })
        }
        if (this.filters.edad) {
          list = list.filter((c) => {
            return c['edad'].match(this.filters.edad)
          })
        }
